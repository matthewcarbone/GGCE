...
class MassSolver(Solver):
    ...
    def spectrum(self, k, w, eta, return_meta=False, pbar=False):
        ...
        # there are limitations: the number of jobs has to be evenly divisible
        # by all the brigades this will be improved in future releases
        if not (1 - len(jobs) % self.brigades):
            logger.critical(
                f"Jobs ({len(jobs)}) cannot be equally divided"
