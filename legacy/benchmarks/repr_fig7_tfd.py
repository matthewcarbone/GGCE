...
if COMM.Get_rank() == 0:
    ...
    ax.plot(
        w_vals, a_func, "b", label=rf"GGCE ($M={cloud_ext_H}, N={bosons_H}$)"
    )
    ...
    plt.savefig(
        os.path.join(
            results_dir, f"repr_fig7_dmrg_M_{cloud_ext_H}_N_{bosons_H}.png"
        ),
        format="png",
    )
