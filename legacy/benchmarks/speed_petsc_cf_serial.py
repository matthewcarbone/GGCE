...
if COMM.Get_rank() == 0:
    ...
    std_error_serial = [
        np.std(serial_times[ii, :], ddof=1) / np.sqrt(len(serial_times[ii, :]))
    ...
    std_error_parallel = [
        np.std(parallel_times[ii, :], ddof=1)
    ...
    xx = np.array(
        [
            ...
            std_error_parallel,
            greens_average_serial,
            greens_average_parallel,
        ]
    ).T
    np.savetxt(
        os.path.join(
            script_dir,
            f"speed_benchmark_M_{str(cloud_ext)}_N_{bosons_min}_{bosons_max}.txt",
        ),
        xx,
    ...
    plt.yticks(size=12)
    plt.title(
        f"Speed comparison for M={cloud_ext}, N=[{bosons_min} to {bosons_max}]",
        fontsize=18,
    )
    ...
    plt.savefig(
        os.path.join(
            script_dir,
            f"benchmark_petsc_M_{str(cloud_ext)}_N_{str(bosons_min)}_{str(bosons_max)}.png",
        ),
        format="png",
        bbox_inches="tight",
    )
