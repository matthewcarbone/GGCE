...
Many of the previous studies analyzed polarons at zero temperature. We include functionality that makes use of the Thermofield Doubles formalism [@suzuki1985thermo; @takahashi1996thermo], which exactly maps any given model at finite temperature to one at zero temperature with couplings to real and fictious phonons, which can then be studied using GGCE. Benchmarks of this approach are ongoing, but preliminary results suggest that the method may be competitive with state-of-the-art methods. A paper with these results is currently in preparation.
