...
Formally, the GGCE functions as an on-the-fly generator of equations of motion for the single-particle Green's function given a set of control parameters (cloud size, total phonon number, and their extensions to systems with multiple phonon modes) and input model parameters (energy scales, coupling strength, etc.). The generated system of equations is then solved numerically in order to obtain the Green's function of interest using a chosen solver.
