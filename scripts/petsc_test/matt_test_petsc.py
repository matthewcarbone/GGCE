...
from ggce.executors.petsc4py.parallel import MassiveSolverMUMPS
...
if __name__ == "__main__":
    ...
    solver = MassiveSolverMUMPS(system=System(model), mpi_comm = COMM, \
                                    brigade_size = 2, root = root_dir)
    ...
    #     dimensionless_coupling_strength=1.,
    # )
    # solver = BigSolverMUMPS(system=System(model), mpi_comm = COMM, \
    #                                                 brigade_size = 1)
    # results = solver.spectrum(k, w, eta=0.05, pbar=True)
    #
    # if COMM.Get_rank() == 0:
    #     results = results.squeeze()
