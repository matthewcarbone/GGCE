...
if COMM.Get_rank() == 0:
    ...
    # now create nested directories for text and iamge respectively
...
for m in range(cloud_min,cloud_max+1):
    for n in range(bosons_min,bosons_max+1):
        ...
        if COMM.Get_rank() == 0:
            ...
            greens_func, times_as_dict = np.array(results).T
            spectrum = np.array([ -s.imag / np.pi for s in greens_func ])
            times = np.array([time_dict['time'][0] for time_dict in times_as_dict])
