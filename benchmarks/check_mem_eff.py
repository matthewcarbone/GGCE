...
model.add_coupling(
    "Holstein", Omega=Omega, M=cloud_ext, N=bosons_max,
...
if COMM.Get_rank() == 0:
    ...
    ax.plot(wgrid, spectrum / spectrum.max(), 'k', label = rf'PETSc ($M={cloud_ext}, N={bosons_max}$)')
    ax.set_ylabel(f"$A(0, \omega)$ [normalized]")
    ax.set_xlabel("$\omega$")
    plt.legend(bbox_to_anchor=(1,1), loc="upper left")
    plt.tight_layout()
    plt.savefig(os.path.join(spectra_vis_path,f'scope_M_{cloud_ext}_N_{bosons_max}.png'), format='png')
    ...
    np.savetxt(os.path.join(text_data_path,f"parallel_results_M_{cloud_ext}_N_{bosons_max}.txt"), xx,\
