...
# article
Omega = np.array([10.,5.,2.,1.,0.5,0.1,0.05,0.01])
coupling = np.linspace(0., 1.2, 30)
hopping = 1.
eta = 0.005
next_lam_offset = 0.1
...
gs_energies = np.zeros((cloud_max-cloud_min+1, bosons_max-bosons_min+1,\
                                                len(Omega), len(coupling)))
    ...
    for n in range(bosons_min,bosons_max+1):
        for ii, omega in enumerate(Omega):
            ...
            for jj, lam in enumerate(coupling):
                ...
                if jj > 0:
                    w0 = gs_energies[m-cloud_min,n-bosons_min,ii,jj-1] - next_lam_offset
            ...
            if COMM.Get_rank() == 0:
                ...
                ax.plot(coupling, gs_energies[m-cloud_min,n-bosons_min,ii,:], label = rf'PETSc ($M={m}, N={n}, \Omega = {omega}$)')
                ax.set_ylim(-3.1,-1.9)
                ax.set_xlim(0, 1.2)
                ...
                xx = np.array([coupling, gs_energies[m-cloud_min,n-bosons_min,ii,:]]).T
                np.savetxt(os.path.join(results_dir,f"gs_energy_vs_lam_M_{m}_N_{n}_om_{omega}.txt"), xx,\
...
if COMM.Get_rank() == 0:
    ...
    # also output the energy data to disk for posterity / postprocess
    xx = np.array([coupling,] + [gs_energies[m,n,l,:] \
                        for l in range(gs_energies.shape[2]) \
                        for m in range(gs_energies.shape[0]) \
                        for n in range(gs_energies.shape[1])]).T
    np.savetxt(os.path.join(results_dir,f"gs_energy_vs_lam_Mmax_{cloud_max}_Nmax_{bosons_max}.txt"), xx,\
